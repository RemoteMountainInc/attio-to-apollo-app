...
// Type declarations for Attio window object
interface AttioApi {
...
function ApolloSync() {
  ...
  const syncFromApollo = async () => {
    ...
    if (!settings || !(settings.apollo_api_key as string)) {
    ...
    try {
      ...
      const response = await fetch('https://api.apollo.io/v1/contacts/search', {
        ...
        body: JSON.stringify({
          ...
          api_key: (settings.apollo_api_key as string) || '',
      ...
      for (const contact of contacts) {
        ...
        try {
          ...
          const attioApi: AttioApi | undefined = window.Attio?.api;
          if (!attioApi) {
            throw new Error('Attio API not available');
          }
  ...
  const syncToApollo = async () => {
    ...
    try {
      ...
      for (const contact of contacts) {
        const contactData = contact as {values?: {email_addresses?: Array<{email_address?: string}>, name?: {first_name?: string, last_name?: string}}}

        const email = contactData.values?.email_addresses?.[0]?.email_address;
        ...
        const apolloData = {
          ...
          first_name: contactData.values?.name?.first_name || '',

          last_name: contactData.values?.name?.last_name || ''
